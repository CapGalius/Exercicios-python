...
while (True):
    ...
    facesDetectadas = classificador.detectMultiScale(imagemCinza, scaleFactor=1.2,minSize=(150,150))

    for (x , y , l, a) in facesDetectadas:
        cv2.rectangle(imagem, (x, y), (x + l, y + a), (0, 0, 255), 3)
        regiao = imagem [y:y+a,x:x+l]
        #regiaoCinzaOlho = cv2.cvtColor(regiao, cv2.COLOR_BGR2GRAY)
        #olhosDetectados = classificaorOlho.detectMultiScale(regiaoCinzaOlho)
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            ...
            cv2.imwrite("Deteccao/fotos/pessoa." +str(id) + "." + str(amostra) + ".jpg",imagemFace)
